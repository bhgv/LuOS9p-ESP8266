...
static const IRAM TValue *luaR_auxfind(const luaR_entry *pentry, const char *k, luaR_numkey nk, unsigned *ppos) {
 ...
 *
 */
const IRAM TValue *luaR_findglobal(const char *name) {
	// Try to get from cache
	#if CONFIG_LUA_RTOS_LUA_USE_ROTABLE_CACHE
	const TValue *res = NULL;
...
int IRAM luaR_findfunction(lua_State *L, const luaR_entry *ptable) {
	const TValue *res = NULL;
...
//if((&_irom0_text_start) <= (uint32_t *)p && (uint32_t *)p <= (&_irom0_text_end))
//printf("luaR_isrotable p=%x, &_irom0_text_start=%x, &_irom0_text_end=%x, %x %x %x\n", p, 
//		&_irom0_text_start, &_irom0_text_end,
//		&_rodata_start, &_rodata_end, &_lit4_end
//		);

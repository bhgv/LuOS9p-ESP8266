...
*/
int sizei (const lpeg_Instruction *i) {
...
void realloccode (lua_State *L, Pattern *p, int nsize) {
  ...
  void *newblock = f(ud, p->code, p->codesize * sizeof(lpeg_Instruction),
                                  nsize * sizeof(lpeg_Instruction));
  if (newblock == NULL && nsize > 0)
    luaL_error(L, "not enough memory");
  p->code = (lpeg_Instruction *)newblock;
  p->codesize = nsize;
}
...
*/
static int finaltarget (lpeg_Instruction *code, int i) {
...
*/
static int finallabel (lpeg_Instruction *code, int i) {
...
static void correctcalls (CompileState *compst, int *positions,
                          int from, int to) {
  int i;
  lpeg_Instruction *code = compst->p->code;
...
** Compile a pattern
*/
lpeg_Instruction *compile (lua_State *L, Pattern *p) {
  CompileState compst;

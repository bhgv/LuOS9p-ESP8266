...
static void printjmp (const lpeg_Instruction *op, const lpeg_Instruction *p) {
  printf("-> %d", (int)(p + (p + 1)->offset - op));
}


void printinst (const lpeg_Instruction *op, const lpeg_Instruction *p) {
...
void printpatt (lpeg_Instruction *p, int n) {
  lpeg_Instruction *op = p;
  while (p < op + n) {
    printinst(op, p);
    p += sizei(p);
  }

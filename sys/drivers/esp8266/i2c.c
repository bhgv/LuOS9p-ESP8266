...
//            sclLevel = GPIO_INPUT_GET(GPIO_ID_PIN(I2C_MASTER_SCL_GPIO));
            sclLevel = GPIO_INPUT_GET(I2C_MASTER_SCL_GPIO);
        } while(sclLevel == 0);
    }
}
...
//    sda_out = GPIO_INPUT_GET(GPIO_ID_PIN(I2C_MASTER_SDA_GPIO));
    sda_out = GPIO_INPUT_GET(I2C_MASTER_SDA_GPIO);
    return sda_out;
}
...
i2c_master_init(void)
{
    ...
    for (i = 0; i < 28; i++) {
        i2c_master_setDC(1, 0);
        i2c_master_wait(I2C_DEF_DELAY);	// sda 1, scl 0
        i2c_master_setDC(1, 1);
        i2c_master_wait(I2C_DEF_DELAY);	// sda 1, scl 1
    }
...
{
    pinSDA = sda; //pin_num[sda];
    pinSCL = scl; //pin_num[scl];

    ETS_GPIO_INTR_DISABLE() ;
//    ETS_INTR_LOCK();

	gpio_pin_opendrain(sda);
	gpio_pin_opendrain(scl);
...
//	taskENTER_CRITICAL();
    ...
    i2c_master_wait(I2C_DEF_DELAY);	// sda 0, scl 1
}
...
i2c_master_stop(void)
{
    ...
    i2c_master_wait(I2C_DEF_DELAY);	// sda 0
    i2c_master_setDC(0, 1);
    i2c_master_wait(I2C_DEF_DELAY);	// sda 0, scl 1
    i2c_master_setDC(1, 1);
    i2c_master_wait(I2C_DEF_DELAY);	// sda 1, scl 1
...
i2c_master_setAck(uint8 level)
{
    ...
    i2c_master_wait(I2C_DEF_DELAY);	// sda level, scl 0
